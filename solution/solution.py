...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            chunk_recall = overlap / len(expected_tokens)
    ...
    def run_full_eval(
        ...
        # Create a minimal QAPair for the result (we don't have the original here)
        # The actual QAPair will be set by BenchmarkRunner
        from dataclasses import replace
        dummy_pair = QAPair(question=question, expected_answer=expected, context=context)
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = f"""You are an expert evaluator. Score the following AI response according to the rubric.
...
- "scores": object mapping each criterion to a score 0.0-1.0
- "reasoning": brief explanation for each score

Example output:
{{
  "scores": {{"accuracy": 0.8, "clarity": 0.9}},
  "reasoning": "The answer is factually correct but could be more concise."
}}"""
        ...
        try:
            ...
            if start >= 0 and end > start:
                ...
                normalized_scores = {k: max(0.0, min(1.0, float(v))) for k, v in scores.items()}
                # Ensure all rubric criteria have scores
        ...
        return {"scores": default_scores, "reasoning": "Failed to parse judge response"}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # Positional bias: first response scores higher than others on average
        first_scores = scores_batch[0].get("scores", {})
        other_scores_list = [s.get("scores", {}) for s in scores_batch[1:]]

        positional_bias = False
        if first_scores and other_scores_list:
            first_avg = sum(first_scores.values()) / len(first_scores)
            other_avgs = [sum(s.values()) / len(s) for s in other_scores_list if s]
            if other_avgs and first_avg > max(other_avgs) + 0.1:
                positional_bias = True

        # Leniency bias: average score > 0.8
        all_scores = []
        ...
        # Severity bias: average score < 0.3
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            result = replace(result, qa_pair=pair)
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        # Average retrieval scores (only non-None)
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        def compute_avg(results):
            ...
            comp = sum(r.completeness for r in results) / n
            return f, rel, comp

        new_f, new_rel, new_comp = compute_avg(new_results)
        base_f, base_rel, base_comp = compute_avg(baseline_results)
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures):
            ...
            suggested_fix = suggestions[i] if i < len(suggestions) else (suggestions[-1] if suggestions else "Review pipeline")
    ...
    def generate_improvement_suggestions(
        ...
        # Prioritize based on failure type frequency
        if categories.get("hallucination", 0) > 0:
            suggestions.append("Implement hallucination checker to filter unsupported claims")
            suggestions.append("Add grounding verification step before returning answers")
        ...
        if categories.get("irrelevant", 0) > 0:
            suggestions.append("Improve prompt clarity with explicit instructions for addressing the question")
            suggestions.append("Add query rewriting step to better match user intent")

        if categories.get("off_topic", 0) > 0:
            suggestions.append("Add intent classification to route questions to appropriate handlers")

        # Generic suggestions if we have fewer than 3
        generic = [
            "Implement retrieval reranking to improve context precision",
            "Add citation requirement in prompt to improve faithfulness",
            "Increase context window for generation to capture more evidence",
        ]
        for g in generic:
        ...
        return suggestions[:5] if suggestions else generic[:3]
